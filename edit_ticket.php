
<!--Indievarie-->

<?php
session_start();
if (!isset($_SESSION['user_name'])) {
    header("Location: signin.php");
    exit();
}

include('config.php'); // Your database connection file

if (isset($_GET['id'])) {
    $id = $_GET['id'];

    // Fetch ticket details
    $sql = "SELECT * FROM tickets WHERE id = '$id'";
    $result = $con->query($sql);
    $ticket = $result->fetch_assoc();

    if (!$ticket) {
        echo "Ticket not found.";
        exit();
    }

    // Handle form submission
    if(isset($_POST['email'], $_POST['phone'], $_POST['issue'])){
        $email = $_POST['email'];
        $phone = $_POST['phone'];
        $issue = $_POST['issue'];

        $sql = "UPDATE tickets SET email = '$email', phone = '$phone', issue = '$issue' WHERE id = '$id'";

        if ($con->query($sql) === TRUE) {
            echo "Ticket updated successfully!";
            header("Location: view_tickets");
            exit();
        } else {
            echo "Error: " . $conn->error;
        }
    }
}
?>

<!DOCTYPE html>
<head>
    <link rel="stylesheet" href="style/styles.css">
    <script src="js/trans.js"></script>
    <title>FIT-GO | Edit Tickets</title>
    <link rel="icon" type="image/x-icon" href="img/logobk.png">

    <script src="js/fitgojs.js"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" rel="stylesheet"><!--to use font awesome library via cdn ,we need it for social media icons in footer-->

    <style>
        body
        {
            background-image:url('img/tic.jpg');
            background-repeat:no-repeat;
            background-size:cover;
            background-attachment: fixed;
        }
        </style>
</head>
<body>
<script>
 function toggleNav() {
    const navLinks = document.getElementById('navbuttons');
    navLinks.classList.toggle('active');
}

// Hide the menu if clicked outside
document.addEventListener('click', function(event) {
    const navLinks = document.getElementById('navbuttons');
    const hamburger = document.querySelector('.hamburger');

    // If the clicked target is not the menu or the hamburger button, hide the menu
    if (!navLinks.contains(event.target) && !hamburger.contains(event.target)) {
        navLinks.classList.remove('active');
    }
});


        
        </script>
    <header>
        <nav>
            <a href="index.php"><img style="width:60px; height:60px;padding: 0;margin-left:10px;"class="logo" src="img/logo.png"></a>
            <div style="height:0;padding: 0; margin: 0;" class="menu-toggle" onclick="toggleMenu()">☰</div>
            <ul class="navbuttons" id="navbuttons">
                <li><a href="index.php" >HOME</a></li>
                <li><a href="aboutus.php">ABOUT US</a></li>
                <li><a href="sessions.php">SESSIONS</a></li>
                <li><a href="train.php">TRAINERS</a></li>
                <li><a href="contact.php"class="active">CONTACT US</a></li>
                <li><a href="blog.php">BLOG</a></li>
            </ul>
            <div class="user-info">
                 <?php

                    if (isset($_SESSION['user_name'])) 
                    {
                        echo '<span class="reg_user">Welcome, ' . htmlspecialchars($_SESSION['user_name']) . '!</span>&ensp;&ensp;&ensp;&ensp;';
                        echo '<div class="profile-btn" style="margin-right:20px;"><i class="fas fa-user-circle"></i></div>';  
                    }
                    else 
                    {
                        echo '<span class="visitor">Welcome, Visitor</span>&ensp;&ensp;&ensp;&ensp;';
                        echo '<a href="signin.php" class="sign-in-btn">Sign In</a>';
                    }
            ?>
            <br>
            <div class="profile-menu hidden">
            <ul>
                <li><a href="feedback.php">My Account</a></li>
                <li><a href="viewfeedback.php">Feedback</a></li>
                <li><a href="sessionreg.php">Registered Sessions</a></li>
                <li><a href="signout.php">Log Out</a></li>
            </ul>
        </div>
     </div>
     <button class="hamburger" onclick="toggleNav()">&#9776;</button>
        </nav>
</header>



<script>
        function toggleNav() {
            const navLinks = document.querySelector('.navbuttons');
            navLinks.classList.toggle('active');
        }
        
</script>
        <center>
        <div style="margin-top:100px">
    <h2>Edit Ticket</h2>
    </div>
    </center>
    
    <div class="contact-form" style="width: 500px; margin-top:80px; margin-bottom:80px;">
    <?php
    echo "<form method='POST' action='edit_ticket.php'>
            <label for='ticket_id'>Ticket ID: </label>
            <input style='background-color:grey; color:white;' type='text' name='username' value='{$ticket['id']}' required readonly><br>



            <label for='email'>Email:</label>
            <input type='email' name='email' value='{$ticket['email']}' required><br>

            <label for='phone'>Phone:</label>
            <input type='tel' name='phone' value='{$ticket['phone']}' required><br>

            <label for='issue'>Issue:</label>
            <textarea name='issue' required>{$ticket['issue']}</textarea><br>

            <button type='submit' class='btn submit'>Update</button>
          </form>";
    ?>
</div>

        
    </form><br>
    <form action="contact.php">
            <button type="submit" class="btn">Go to Contact Page</button>
        </form>
</div>


<footer>
      <div class="footer-container">
        <div class="footer-sec contact">
          <h4>Contact us</h4>
          <p><i class="fas fa-phone-alt"></i> +94 98765 43210</p>
          <p><i class="fas fa-envelope"></i> info@example.com</p>
          <p><i class="fas fa-map-marker-alt"></i> type something here</p>
        </div>
        
        <div class="footer-sec about">
          <h4>About Us</h4>
          <ul>
            <li><a href="#">Terms & Conditions</a></li>
            <li><a href="#">Privacy Policy</a></li>
            <li><a href="#">Monitored Tracks</a></li>
            <li><a href="#">Equipment Details</a></li>
          </ul>
        </div>
        
        <div class="footer-sec menu">
          <h4>Menu</h4>
          <ul>
            <li><a href="#">Home</a></li>
            <li><a href="#">Sessions</a></li>
            <li><a href="#">Products</a></li>
            <li><a href="#">About Us</a></li>
          </ul>
        </div>
        
        <div class="footer-sec newsletter">
          <h4>Subscribe</h4>
          <form>
            <input type="email" placeholder="mail@example.com" required>
            <button type="submit">Subscribe</button>
          </form>
          <p>type something.</p>
          <img src="logo.png" alt="Logo" class="footer-logo">
        </div>
      </div>
      <div class="footer-bottom">
        <p>© FITGO | All Rights Reserved</p>
        <div class="social-icons">
          <a href="#"><i class="fab fa-instagram"></i></a>
          <a href="#"><i class="fab fa-facebook"></i></a>
          <a href="#"><i class="fab fa-youtube"></i></a>
        </div>
      </div>
</footer>


</body>
</html>

