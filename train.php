...
// Fetch trainer data
$sql = "SELECT fname, lname, specialization , profile_pic,username , country  FROM trainer";
...
document.addEventListener('click', function(event) {
    ...
    <header>
        <nav>
            ...
            <ul class="navbuttons" id="navbuttons">
                ...
                <li><a href="train.php" class="active">TRAINERS</a></li>
    ...
    <section class="trainerss-hero">
        <div class="heroo-content">
            <h1>Meet Our Personal Trainers</h1>
            <p>Our expert trainers offer personalized training programs tailored to your fitness goals.</p>
        </div>
    </section>

    <section class="trainers">
    <div class="trainers-container"> <!-- Change class name -->
        <?php
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo '<div class="trainer-card">';
                echo '<img src="' . $row['profile_pic'] . '" alt="' . $row['fname'] . ' ' . $row['lname'] . '">';
                echo '<h3>' . $row['fname'] . ' ' . $row['lname'] . '</h3>';
                echo '<h2>' . $row['username'] . '</h2>';
                echo '<p>Country: ' . $row['country'] . '</p>';
                echo '<p>Specialization: ' . $row['specialization'] . '</p>';

                echo '<a href="TrainerPage.php" class="btnt">View Profile</a>';
                echo '</div>';
            }
        } else {
